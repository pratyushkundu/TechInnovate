  ...
  const footerSections = [
    ...
    },
    {
      ...
      links: [
        ...
        { name: "Status", path: "#" },
        { name: "Privacy Policy", path: "/privacy" },   // 👈 route to privacy
        { name: "Terms of Service", path: "/terms" }    // 👈 route to terms
      ]
    }
  ];

  const socialLinks = [
    { icon: Linkedin, href: "https://www.linkedin.com/company/hukitola09/?viewAsMember=true", iconColor: "text-tech-purple" },
    { icon: Twitter, href: "#", iconColor: "text-tech-blue" },
    { icon: Github, href: "https://github.com/pratyushkundu", iconColor: "text-tech-green" }
  ];
    ...
    <footer className="bg-tech-dark dark:bg-tech-dark border-t border-slate-700/50 py-16" data-testid="footer">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex space-x-4">
              {socialLinks.map((social, index) => (
                <a
                  ...
                  href={social.href}
                  target="_blank"
                  className={`text-slate-400 ${social.iconColor} transition-colors duration-300`}
                  data-testid={`footer-social-${index}`}
                >
                  <social.icon className="w-6 h-6" />
                </a>
        ...
        <div className="border-t border-slate-700/50 pt-8 flex flex-col md:flex-row justify-between items-center">
          ...
          <p className="text-slate-400 dark:text-slate-400 text-sm mt-4 md:mt-0">
            Designed and developed with <span className="text-red-500">Hukitola Solutions</span> for the future of technology
          </p>
        </div>
      </div>
